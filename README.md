...
در نسخه تستی فعلی، فایل واقعی `public/api/hesabfa/config.php` داخل پروژه قرار دارد تا بدون بک اند جداگانه اتصال حسابفا تست شود.
بعد از ساخت بک اند اصلی، این فایل باید حذف شود و اطلاعات حسابفا فقط در بک اند نگهداری شود.
